...
async def bgmi(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(f'Flooding parameters set: {target_ip}:{port} for {duration} seconds with {DEFAULT_THREADS} threads.OWMER- @RAHUL_DDOS_B.')
...
async def stop(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if flooding_process is None:
        await update.message.reply_text('No flooding process is running.OWNER @Shsonu...')
        return
